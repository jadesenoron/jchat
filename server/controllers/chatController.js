...
function initializeChat(userIdA, userIdB) {
  return new Promise((resolve, _) => {
    Chat.findOne({
      'users._id': { $all: [userIdA.toString(), userIdB.toString()] }
    }).then((oldchat) => {
      if (!oldchat) {
...
function getObjectIdsBySearch(search) {
  return new Promise((resolve) => {
    Promise.all([
      User.find({ username: { $regex: search, $options: 'i' }}).select({ _id: 1 }),
      User.find({ firstname: { $regex: search, $options: 'i' }}).select({ _id: 1 }),
      User.find({ middlename: { $regex: search, $options: 'i' }}).select({ _id: 1 }),
      User.find({ lastname: { $regex: search, $options: 'i' }}).select({ _id: 1 })
    ]).then((results) => {
      let result = []
  ...
  try {
    switch (query) {
      case 'conversation': {
        ...
        const hasUserId = chatdoc.users.filter(({_id}) => _id.toString() === from_user).length === 1
        if (!hasUserId) {
          return res.status(403).json('Invalid Conversation!')
        }
        // update seen
        const myuserindex = [...chatdoc.users].map(({_id}, i) => _id.toString() === from_user ? i : null).filter(v => v !== null)[0]
      ...
      case 'search': {
        ...
        const chatdocs = await Chat.find({
          'users._id': { $all: [from_user], $in: [...to_userids] },
          lastUpdated: { $gt: 0 }
...
export const sendChat = async (req, res, next) => {
  ...
  try {
    if (!chatid) {
      ...
      chat = await Chat.findById(chatid).where('users._id').all([from_userid])
    }
    if (!chat) {
      return res.status(500).json('No conversation found!')
    }
    const datenow = new Date(Date.now())
    senderdoc = senderdoc ? senderdoc : await User.findById(from_userid).select('username firstname lastname')
    receiverdoc = receiverdoc ? receiverdoc : await User.findById(chat.users.filter(v => v._id.toString() !== from_userid)[0]._id.toString()).select('username')
